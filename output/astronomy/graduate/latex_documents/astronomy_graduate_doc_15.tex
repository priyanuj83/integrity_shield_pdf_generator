...
\item True or False: Water-ice is distributed evenly across Mercury's surface, not limited to the polar regions.
\item True or False: In order to observe a total solar eclipse, the Moon's umbra must intersect the location of the observer on Earth.
\item True or False: The Schwarzschild radius is primarily used to describe the characteristics of pulsars in astrophysics.
...
\begin{enumerate}[label=\arabic*.]
\setcounter{enumi}{10}
\item Discuss the implications for solar and lunar eclipses if the Moon's orbital plane aligned precisely with the ecliptic plane, analyzing the effects on their frequency and visibility.
\item Discuss the significance of temperature measurement in astronomy, specifically comparing the Celsius and Kelvin scales, and calculate the equivalent of 25^\ensuremath{\circC} in Kelvin.
\end{enumerate}
